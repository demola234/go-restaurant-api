...
func GetOrderItems() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		defer cancel()
		if err != nil {
			c.JSON(http.StatusInternalServerError, gin.H{"error": "Error Occurred fetching all the orders"})
			return
		}
...
func GetOrderItem() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		if err != nil {
			c.JSON(http.StatusInternalServerError, gin.H{"error": "An Error Occurred Fetching data"})
			return
		}
		c.JSON(http.StatusOK, allOrderItems)
	}
...
func GetOrderItemsByOrder() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		defer cancel()
		if err != nil {
			c.JSON(http.StatusInternalServerError, gin.H{"error": "An Error Occurred getting Ordered Items"})
			return
		}
...
func ItemsByOrder(id string) (OrderItems []primitive.M, err error) {
	...
	unwindOrderStage := bson.D{{"$unwind", bson.D{{"path", "$food"}, {"preserveNullAndEmptyArray", true}}}}
	...
	groupState := bson.D{{"$group", bson.D{{"_id", bson.D{{"order_id", "$order_id"}, {"table_id", "$table_id"}, {"table_number", "$table_number"}}}, {"payment_due", bson.D{{"$sum", "$amount"}}}, {"total_count", bson.D{{"$sum", 1}}}, {"order_items", "$$PUSH"}}}}
...
func UpdateOrderItem() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		defer cancel()
		if err != nil {
			msg := "Orders items update failed"
